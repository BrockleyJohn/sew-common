...
/*  sew_ajax
	...
	catalog version (do not use in admin)
...
*/
  ...
  require_once('includes/functions/sew_ajax_functions.php'); // in case not in app top...
  ...
  } elseif (! (isset($_REQUEST['checksess']) && $_REQUEST['checksess'] == $sessiontoken) {
	header('HTTP/1.0 403 Forbidden', true, 403);
	header('Content-Type: application/json; charset=utf-8');
	exit(json_encode(['error' => 'No matching session']));
  }
